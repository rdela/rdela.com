---
layout: layouts/page.vto
title: Links
excerpt: "My links page: Hyperlinks to me around the web (studios I co-founded & featured accounts elsewhere) + my favorite websites"
menu:
  visible: true
  order: 3
---

Welcome to my links&nbsp;page&nbsp;🔗📄

Hyperlinks to me around the web + my favorite websites

## Me Around the Web 

🔄🕸

### Studios I co-founded

- [Famebot](https://famebot.com/)

- [Artist Activist](https://artact.io/)

### Featured accounts elsewhere

- [github.com/rdela](https://github.com/rdela)
 
- [@rdela@mastodon.social](https://mastodon.social/@rdela)
 
- [@famebot.com on Bluesky](https://bsky.app/profile/famebot.com)
 
- [@artact.io on Bluesky](https://bsky.app/profile/artact.io)

## Keep on Readin’ On

📖📚

<figure><a
  href="https://readtheprintedword.org/"
  style="display: block; margin-block-start: 12px"><img
  src="https://readtheprintedword.org/rtpw-button4-200x92white.png"
  alt="Read the Printed Word!"
  width="132"
  height="78"
  style="background-color: black; padding: 16px; border-radius: 16px;" /><br />Read the Printed&nbsp;Word</a></figure>

## Support *Knowledge for Everyone*

🧠🌐

<figure><a
  href="https://wikitech.wikimedia.org/wiki/Main_Page"
  style="display: block; margin-block-start: 24px"><img
  src="/svg/wikicorn.svg"
  alt="wikicorn"
  width="150"
  height="150" /></a><figcaption class="svg-label">

### [Wikitech](https://wikitech.wikimedia.org/wiki/Main_Page)

…and the entire [*movement*](https://meta.wikimedia.org/wiki/Wikimedia_movement) around [Wikimedia](https://www.wikimedia.org/)

Technical Collective Educational Content&nbsp;Development

</figcaption></figure>

<br />

<figure><a
  href="https://archive.org/"
  style="display: block; margin-block-start: 24px"><img
  src="/svg/internet-archive-inv.svg"
  alt="wikicorn"
  width="150"
  height="145.775"
  style="background-color: black; padding: 16px; border-radius: 16px;" /></a><figcaption class="svg-label">

### [Internet Archive](https://archive.org/)

Search millions of free books, movies, software, music, and the history of hundreds of billions of web&nbsp;pages.

<small>(Image source: [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:Internet_Archive_logo_and_wordmark.svg))</small>

</figcaption></figure>

<br />

<figure><a
  href="https://developer.mozilla.org/en-US/docs/Web"
  style="display: block; margin-block-start: 24px"><img
  src="/svg/mdn.svg"
  alt="MDN dino"
  width="544.5" 
  height="132"
  style="background-color: black; padding: 16px; border-radius: 16px;" /></a><figcaption class="svg-label">

### [Mozilla Developer Network](https://developer.mozilla.org/en-US/docs/Web)

Open Web Technology Resource

</figcaption></figure>
